...
private fun parseCompletedIndexedTranslationPrefix(
    content: String,
    expectedSubtitles: List<SubtitleEntry>,
    format: SubtitleFormat,
    expectedStartPosition: Int
...
class AiTranslator(
    ...
    private suspend fun translateBatch(
        ...
        val progressTracker = TimedStreamProgress(
            ...
            format = subtitleFormat,
            expectedStartPosition = batchStartPosition,
    ...
    private class TimedStreamProgress(
        ...
        private val format: SubtitleFormat,
        private val expectedStartPosition: Int,
        ...
        fun finish() {
            consumeCurrentLine()
            reportProgress(countCompletedTranslations(completedContent.toString(), false))
        }

        fun completedTranslations(): List<String> = if (format == SubtitleFormat.SRT) {
            parseCompletedTimedTranslationPrefix(completedContent.toString(), expectedSubtitles)
        } else {
            parseCompletedIndexedTranslationPrefix(
        ...
        private fun consumeCurrentLine() {
            ...
            val isCueLine = when (format) {
                SubtitleFormat.LRC -> LRC_TIMED_LINE.containsMatchIn(line)
                SubtitleFormat.VTT -> VTT_TIMED_LINE.matches(line)
                else -> timedSubtitleLineRange(line) in expectedTimeRanges
            }
            if (isCueLine) {
                val completedCount = countCompletedTranslations(completedContent.toString())
        ...
        private fun countCompletedTranslations(
            content: String,
            ...
            return (if (excludeTrailingBlock) blocks.dropLast(1) else blocks)
                .count { block ->
                    val expected = expectedKeys[block.sequence] ?: return@count false
                    block.startTimeMs == expected.startTimeMs &&
                        (format != SubtitleFormat.VTT || block.endTimeMs == expected.endTimeMs)
                }
        }
